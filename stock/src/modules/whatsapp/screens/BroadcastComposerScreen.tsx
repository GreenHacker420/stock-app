...
function isBroadcastReady(template: WaTemplate) {
  ...
  return ["NONE", "TEXT", "IMAGE", "VIDEO", "DOCUMENT"].includes(format);
}
...
function templateDefinition(template: WaTemplate, values: Record<string, string>): Partial<WaTemplateDefinition> {
  ...
  return {
    ...
    mappings: template.variableMappings.map((mapping) => ({
      ...
      sampleValue: values[mapping.id] || mapping.fallbackValue || mapping.sampleValue,
...
function buildTemplateVariables(template: WaTemplate, values: Record<string, string>, media: HeaderAsset | null) {
  const mapped = (component: "HEADER" | "BODY") => template.variableMappings
    .filter((mapping) => mapping.component === component)
    .sort((left, right) => left.position - right.position)
    .map((mapping) => values[mapping.id] || mapping.fallbackValue || mapping.attribute?.fallbackValue || "");
  const header = mapped("HEADER");
  const body = mapped("BODY");
  return {
    ...(header.length ? { header } : {}),
    ...(body.length ? { body } : {}),
    ...(media ? { headerAssetId: media.id, headerFileName: media.fileName } : {}),
  };
}
...
function ContactRow({
  ...
  return (
    <Pressable onPress={onToggle} style={({ pressed }) => [styles.contactRow, pressed && styles.rowPressed]}>
      ...
      <View style={styles.contactText}>
        <Text style={styles.contactName} numberOfLines={1}>{item.name || formatWhatsAppPhone(item.phone)}</Text>
        <Text style={styles.contactPhone}>{formatWhatsAppPhone(item.phone)}</Text>
      </View>
      {item.tag !== "NONE" && (
        <Text style={styles.contactTag}>{item.tag === "BUSINESS" ? "Business" : "Regular"}</Text>
      )}
...
export function BroadcastComposerScreen() {
  ...
  const addManualRecipient = () => {
    ...
    setManualRecipients((current) => [...current, {
  ...
  const chooseTemplate = (template: WaTemplate) => {
    ...
    setMappingValues(Object.fromEntries(
      template.variableMappings
        .filter((mapping) => mapping.component === "HEADER" || mapping.component === "BODY")
  ...
  const requiredMappingsMissing = useMemo(() => {
    ...
    return selectedTemplate.variableMappings.filter((mapping) =>
      (mapping.component === "HEADER" || mapping.component === "BODY")
      && mapping.required
      && !(mappingValues[mapping.id] || mapping.fallbackValue || mapping.attribute?.fallbackValue),
    );
  }, [mappingValues, selectedTemplate]);
  const mediaRequired = ["IMAGE", "VIDEO", "DOCUMENT"].includes(headerFormat(selectedTemplate));

  const canContinueAudience = audienceCount > 0;
  ...
  const sendMutation = useMutation({
    mutationFn: async () => {
      ...
      const broadcast = await createWaBroadcast(token, {
        ...
        templateVariables: buildTemplateVariables(selectedTemplate, mappingValues, headerAsset),
      });
  ...
  const continueForward = () => {
    if (step === 0 && !canContinueAudience) {
      Alert.alert("Choose recipients", "Select at least one contact or add a number manually.");
      return;
    }
    if (step === 1 && !canContinueMessage) {
      if (!selectedTemplate) Alert.alert("Choose a template", "Select an approved WhatsApp template.");
      else if (requiredMappingsMissing.length) Alert.alert("Complete variables", "Fill every required template variable.");
      else Alert.alert("Choose media", `This template requires a ${headerFormat(selectedTemplate).toLowerCase()} header.`);
      return;
    }
    ...
    <View style={styles.flex}>
      <View style={styles.searchArea}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterRow}>
          {(["ALL", "BUSINESS", "REGULAR", "NONE"] as const).map((filter) => (
            <Pressable
              ...
              style={[styles.filterChip, tagFilter === filter && styles.filterChipActive]}
            >
              <Text style={[styles.filterChipText, tagFilter === filter && styles.filterChipTextActive]}>
        ...
        <Pressable onPress={toggleAllFiltered} style={styles.selectAllLine}>
          <View style={[styles.selectionCircle, allFilteredSelected && styles.selectionCircleSelected]}>
            {allFilteredSelected && <MaterialCommunityIcons name="check" size={13} color="#fff" />}
          </View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.manualStrip}>
            {manualRecipients.map((recipient) => (
              <View key={recipient.id} style={styles.manualPill}>
                <Text style={styles.manualPillText} numberOfLines={1}>{recipient.name}</Text>
                <TouchableOpacity onPress={() => setManualRecipients((current) => current.filter((item) => item.id !== recipient.id))}>
                  <MaterialCommunityIcons name="close" size={14} color={waColors.textSecondary} />
                </TouchableOpacity>
              </View>
            ))}
          </ScrollView>
      ...
      {contactsQuery.isLoading ? (
        <View style={styles.center}><ActivityIndicator color={waColors.green} /></View>
      ) : (
        <FlashList
          data={contacts}
          ...
          ListEmptyComponent={
            <View style={styles.emptyContacts}>
              <MaterialCommunityIcons name="contacts-outline" size={40} color={waColors.textMuted} />
              <Text style={styles.emptyTitle}>No local contacts yet</Text>
              <Text style={styles.emptyCopy}>Refresh device contacts. They remain on this phone until you select a campaign audience.</Text>
              <Button mode="outlined" icon="contacts-outline" onPress={() => importMutation.mutate()} loading={importMutation.isPending}>
                Refresh contacts
              </Button>
            </View>
          }
          ListFooterComponent={contactsQuery.isFetchingNextPage ? <ActivityIndicator style={styles.footerLoader} color={waColors.green} /> : null}
          renderItem={({ item }) => (
            <ContactRow item={item} selected={selectedIds.has(item.id)} onToggle={() => toggleContact(item.id)} />
          )}
        />
      )}
    </View>
    ...
    <View style={styles.flex}>
      ...
      {templatesQuery.isLoading ? (
        ...
        <FlashList
          ...
          renderItem={({ item }) => (
            <Pressable onPress={() => chooseTemplate(item)} style={({ pressed }) => [styles.templateRow, pressed && styles.rowPressed]}>
              <View style={styles.templateIcon}>
                <MaterialCommunityIcons name="message-text-outline" size={21} color={waColors.greenDark} />
              </View>
              <View style={styles.templateBody}>
                ...
                <Text style={styles.templateMeta}>{item.language} · {item.category.toLowerCase()} · {headerFormat(item).toLowerCase()} header</Text>
              </View>
          ...
          ListEmptyComponent={
            <View style={styles.emptyContacts}>
              <MaterialCommunityIcons name="message-alert-outline" size={40} color={waColors.textMuted} />
              <Text style={styles.emptyTitle}>No broadcast-ready templates</Text>
              <Text style={styles.emptyCopy}>Use an approved text, image, video, or document template. Carousel and location templates are excluded for now.</Text>
            </View>
          }
        />
      )}
  ...
  const renderMessageEditor = () => {
    ...
    const mappings = selectedTemplate.variableMappings
      .filter((mapping) => mapping.component === "HEADER" || mapping.component === "BODY")
      .sort((left, right) => left.component.localeCompare(right.component) || left.position - right.position);
    return (
      <ScrollView style={styles.flex} contentContainerStyle={styles.editorContent} keyboardShouldPersistTaps="handled">
        <Pressable onPress={() => setSelectedTemplate(null)} style={styles.changeTemplate}>
          <MaterialCommunityIcons name="arrow-left" size={17} color={waColors.greenDark} />
          <Text style={styles.changeTemplateText}>Change template</Text>
          <Text style={styles.changeTemplateName} numberOfLines={1}>{selectedTemplate.name}</Text>
        </Pressable>

        <WhatsAppTemplatePreview definition={templateDefinition(selectedTemplate, mappingValues)} />

        {["IMAGE", "VIDEO", "DOCUMENT"].includes(format) && (
          <View style={styles.editorSection}>
            <Text style={styles.editorLabel}>{format.toLowerCase()} header</Text>
            <Text style={styles.editorHint}>Upload once; the same WhatsApp media asset is reused for every recipient.</Text>
            <Pressable onPress={pickHeaderMedia} style={styles.mediaPicker} disabled={uploadingHeader}>
              <View style={styles.mediaPickerIcon}>
                <MaterialCommunityIcons
                  name={format === "DOCUMENT" ? "file-document-outline" : format === "VIDEO" ? "video-outline" : "image-outline"}
                  size={23}
                  color={waColors.greenDark}
                />
              </View>
              <View style={styles.mediaPickerText}>
                <Text style={styles.mediaPickerTitle}>{headerAsset?.fileName || `Choose ${format.toLowerCase()}`}</Text>
                <Text style={styles.mediaPickerSub}>{headerAsset ? "Ready for broadcast" : "Required by this approved template"}</Text>
              </View>
              {uploadingHeader ? <ActivityIndicator color={waColors.green} /> : <MaterialCommunityIcons name="chevron-right" size={22} color={waColors.textMuted} />}
            </Pressable>
          </View>
        )}
        ...
        {mappings.length > 0 && (
          <View style={styles.editorSection}>
            ...
            <Text style={styles.editorHint}>Recipient tokens resolve per selected number without creating a CRM customer.</Text>
            {mappings.map((mapping) => (
              <View key={mapping.id} style={styles.variableBlock}>
                <View style={styles.variableHeader}>
                  <Text style={styles.variableName}>{mapping.component} {"{{"}{mapping.position}{"}}"}</Text>
                  {!!mapping.attribute?.label && <Text style={styles.variableAttribute}>{mapping.attribute.label}</Text>}
                </View>
                <TextInput
                  mode="outlined"
                  value={mappingValues[mapping.id] || ""}
                  placeholder={mapping.fallbackValue || mapping.attribute?.fallbackValue || "Enter value"}
                  onChangeText={(value) => setMappingValues((current) => ({ ...current, [mapping.id]: value }))}
                  style={styles.variableInput}
                  outlineStyle={styles.inputOutline}
                />
                <View style={styles.tokenRow}>
                  <Pressable onPress={() => setMappingValues((current) => ({ ...current, [mapping.id]: "{{recipient.name}}" }))} style={styles.tokenChip}>
                    <Text style={styles.tokenText}>Recipient name</Text>
                  </Pressable>
                  <Pressable onPress={() => setMappingValues((current) => ({ ...current, [mapping.id]: "{{recipient.phone}}" }))} style={styles.tokenChip}>
    ...
    <ScrollView style={styles.flex} contentContainerStyle={styles.reviewContent} keyboardShouldPersistTaps="handled">
      <View style={styles.reviewHero}>
        <View style={styles.reviewIcon}>
          <MaterialCommunityIcons name="bullhorn-outline" size={26} color={waColors.greenDark} />
        </View>
        <View style={styles.reviewHeroText}>
          <Text style={styles.reviewTitle}>Ready to queue</Text>
          <Text style={styles.reviewSubtitle}>The API request returns immediately; BullMQ handles recipient sends and retries.</Text>
        </View>
      </View>
      ...
      <View style={styles.reviewLine}>
        <Text style={styles.reviewLineLabel}>Audience</Text>
        <Text style={styles.reviewLineValue}>{audienceCount} selected</Text>
      </View>
      <View style={styles.reviewLine}>
        <Text style={styles.reviewLineLabel}>Device contacts</Text>
        <Text style={styles.reviewLineValue}>{selectedIds.size}</Text>
      </View>
      <View style={styles.reviewLine}>
        <Text style={styles.reviewLineLabel}>Manual numbers</Text>
        <Text style={styles.reviewLineValue}>{manualRecipients.length}</Text>
      </View>
      <View style={styles.reviewLine}>
        <Text style={styles.reviewLineLabel}>Template</Text>
        <Text style={styles.reviewLineValue} numberOfLines={1}>{selectedTemplate?.name}</Text>
      </View>
      <View style={styles.reviewLine}>
        <Text style={styles.reviewLineLabel}>Header</Text>
        <Text style={styles.reviewLineValue}>{headerFormat(selectedTemplate).toLowerCase()}</Text>
      </View>

      <View style={styles.privacyNote}>
        <MaterialCommunityIcons name="shield-lock-outline" size={20} color={waColors.greenDark} />
        <View style={styles.privacyText}>
          <Text style={styles.privacyTitle}>Selected contacts only</Text>
          <Text style={styles.privacyCopy}>Search and filtering happened in local SQLite. Only the final selected recipient snapshot is uploaded for this campaign.</Text>
        </View>
      </View>
  ...
  return (
    <View style={styles.screen}>
        ...
        {STEPS.map((label, index) => (
          <View key={label} style={styles.stepItem}>
            <View style={[styles.stepDot, index <= step && styles.stepDotActive]}>
              {index < step ? <MaterialCommunityIcons name="check" size={12} color="#fff" /> : <Text style={[styles.stepNumber, index <= step && styles.stepNumberActive]}>{index + 1}</Text>}
            </View>
      ...
      {step === 0 ? renderAudience() : step === 1 ? renderMessageEditor() : renderReview()}

      <View style={styles.bottomBar}>
        <View style={styles.bottomInfo}>
          {step === 0 && <Text style={styles.bottomPrimary}>{audienceCount} selected</Text>}
          {step === 1 && <Text style={styles.bottomPrimary}>{selectedTemplate ? selectedTemplate.name : "Choose template"}</Text>}
          {step === 2 && <Text style={styles.bottomPrimary}>{audienceCount} recipients</Text>}
          <Text style={styles.bottomSecondary}>
            {step === 0 ? `${statsQuery.data?.total || 0} contacts stored locally` : step === 1 ? "Approved templates only" : "Queued through BullMQ"}
          </Text>
        </View>
        {step > 0 && (
          <Button mode="text" onPress={() => setStep((current) => current - 1)} disabled={sendMutation.isPending} textColor={waColors.textSecondary}>
            Back
          </Button>
        )}
        {step < 2 ? (
          <Button mode="contained" onPress={continueForward} buttonColor={waColors.greenDark} textColor="#fff" contentStyle={styles.continueButtonContent}>
            Continue
          </Button>
        ) : (
          <Button
          ...
          >
            {sendMutation.isPending && uploadProgress > 0 ? `${Math.round(uploadProgress * 100)}%` : "Send"}
          </Button>
        )}
      </View>
      ...
      <Portal>
        <Dialog visible={manualDialog} onDismiss={() => setManualDialog(false)} style={styles.dialog}>
          ...
          <Dialog.Content>
            <Text style={styles.dialogCopy}>This number is added only to this broadcast audience.</Text>
            <TextInput mode="outlined" label="Name (optional)" value={manualName} onChangeText={setManualName} style={styles.dialogInput} />
            <TextInput mode="outlined" label="Phone number" value={manualPhone} onChangeText={setManualPhone} keyboardType="phone-pad" />
          </Dialog.Content>
          <Dialog.Actions>
...
const styles = StyleSheet.create({
  ...
  stepper: { height: 60, flexDirection: "row", alignItems: "center", paddingHorizontal: 18, borderBottomWidth: 1, borderBottomColor: waColors.border },
  stepItem: { flex: 1, flexDirection: "row", alignItems: "center" },
  stepDot: { width: 24, height: 24, borderRadius: 12, alignItems: "center", justifyContent: "center", backgroundColor: waColors.surfaceMuted, borderWidth: 1, borderColor: waColors.border },
  ...
  selectAllText: { marginLeft: 10, flex: 1, fontSize: 12, fontWeight: "750", color: waColors.text },
  ...
  emptyTitle: { marginTop: 12, fontSize: 16, fontWeight: "850", color: waColors.text },
  ...
  templateTitle: { fontSize: 14, fontWeight: "850", color: waColors.text },
  ...
  editorLabel: { fontSize: 14, fontWeight: "850", color: waColors.text },
  ...
  variableName: { fontSize: 11, fontWeight: "850", color: waColors.text },
  ...
  privacyTitle: { fontSize: 12, fontWeight: "850", color: waColors.greenDark },
  ...
  bottomPrimary: { fontSize: 13, fontWeight: "850", color: waColors.text },
