        ...
        <View style={styles.chatBody}>
          <View style={styles.titleRow}>
            <View style={styles.nameRow}>
              <Text style={[styles.name, active && styles.nameActive]} numberOfLines={1}>{displayName}</Text>
              {item.isPinned && <MaterialCommunityIcons name="pin" size={14} color={waColors.textMuted} />}
              {item.isMuted && <MaterialCommunityIcons name="bell-off-outline" size={14} color={waColors.textMuted} />}
            </View>
          ...
          <View style={styles.previewRow}>
            ...
            <Text style={[styles.preview, active && styles.previewActive]} numberOfLines={1}>{messagePreview(lastMessage)}</Text>
            {active && (
    ...
    <View style={styles.screen}>
      <View style={styles.topPanel}>
        <View style={styles.connectionLine}>
          ...
          <Text style={styles.conversationCount}>{query.conversations.length} chats</Text>
        </View>
      ...
      {query.isPending ? (
        ...
        <FlashList
          ...
          onLoad={({ elapsedTimeInMs }) => {
            if (isFocused) markWhatsAppListMeasurement("list-drawn", `flashList=${Math.round(elapsedTimeInMs)}`);
          }}
          refreshControl={
            <RefreshControl
          ...
          ListFooterComponent={query.isFetchingNextPage ? <ActivityIndicator color={waColors.green} style={styles.pageLoader} /> : null}
          ListEmptyComponent={
            <EmptyState
              ...
              subtitle={search ? "Try another name, phone number, or message." : "Start a conversation or wait for a customer to message your business."}
            />
          }
        />
      )}
      ...
      <FAB
        ...
        onPress={() => navigation.navigate("ContactBook", { shopId, integrationId, phoneNumberId })}
      />
          ...
          <Pressable style={styles.actionSheet} onPress={(event) => event.stopPropagation()}>
            ...
            <View style={styles.sheetContact}>
              <View style={[styles.sheetAvatar, { backgroundColor: avatarColor(selected?.phone || "") }]}>
                <Text style={styles.sheetAvatarText}>{initials(selected?.contactName || selected?.phone)}</Text>
              </View>
            ...
            <Pressable style={styles.sheetAction} onPress={() => selected && controlMutation.mutate({ kind: "pin", conversation: selected })}>
              <MaterialCommunityIcons name={selected?.isPinned ? "pin-off-outline" : "pin-outline"} size={23} color={waColors.text} />
              <Text style={styles.sheetActionText}>{selected?.isPinned ? "Unpin chat" : "Pin chat"}</Text>
            </Pressable>
            <Pressable style={styles.sheetAction} onPress={() => selected && controlMutation.mutate({ kind: "mute", conversation: selected })}>
              <MaterialCommunityIcons name={selected?.isMuted ? "bell-outline" : "bell-off-outline"} size={23} color={waColors.text} />
              <Text style={styles.sheetActionText}>{selected?.isMuted ? "Unmute notifications" : "Mute notifications"}</Text>
            </Pressable>
            <Pressable
              style={styles.sheetAction}
              onPress={() => selected && archiveMutation.mutate({ id: selected.id, archive: !selected.isArchived })}
            >
              <MaterialCommunityIcons name={selected?.isArchived ? "archive-arrow-up-outline" : "archive-arrow-down-outline"} size={23} color={waColors.text} />
              <Text style={styles.sheetActionText}>{selected?.isArchived ? "Move to chats" : "Archive chat"}</Text>
            </Pressable>
            <Pressable
              style={styles.sheetAction}
              onPress={() => {
...
const styles = StyleSheet.create({
  ...
  topPanel: { paddingTop: 8, paddingBottom: 10, backgroundColor: "#fff", borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: waColors.border },
  connectionLine: { minHeight: 24, flexDirection: "row", alignItems: "center", paddingHorizontal: 16, gap: 7 },
  ...
  search: { height: 46, marginHorizontal: 12, marginTop: 5, borderRadius: 16, backgroundColor: "#f2f5f4" },
  searchInput: { minHeight: 46, fontSize: 15 },
  filters: { paddingHorizontal: 12, paddingTop: 10, gap: 8 },
  filterChip: { minHeight: 34, paddingHorizontal: 13, borderRadius: 17, flexDirection: "row", alignItems: "center", gap: 7, borderWidth: 1, borderColor: "#dce4e1", backgroundColor: "#fff" },
  ...
  filterCount: { minWidth: 19, height: 19, paddingHorizontal: 5, borderRadius: 10, alignItems: "center", justifyContent: "center", backgroundColor: "#edf1f0" },
  ...
  archivedRow: { minHeight: 56, paddingHorizontal: 17, flexDirection: "row", alignItems: "center", gap: 12, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: waColors.border },
  archivedIcon: { width: 38, height: 38, borderRadius: 13, alignItems: "center", justifyContent: "center", backgroundColor: "#e9f7ef" },
  ...
  chatRow: { minHeight: 78, paddingLeft: 13, flexDirection: "row", alignItems: "center", backgroundColor: "#fff" },
  ...
  avatarActivity: { position: "absolute", right: -1, bottom: -1, width: 14, height: 14, borderRadius: 7, borderWidth: 3, borderColor: "#fff", backgroundColor: waColors.greenBright },
  chatBody: { flex: 1, minWidth: 0, minHeight: 78, marginLeft: 12, paddingRight: 13, justifyContent: "center", borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: waColors.border },
  ...
  unreadBadge: { minWidth: 21, height: 21, paddingHorizontal: 5, borderRadius: 11, alignItems: "center", justifyContent: "center", backgroundColor: waColors.greenBright },
  ...
  toolsMenu: { width: 230, paddingVertical: 7, borderRadius: 18, backgroundColor: "#fff", boxShadow: "0 8px 28px rgba(15, 23, 42, 0.18)" },
  ...
  actionSheet: { paddingHorizontal: 18, paddingTop: 9, paddingBottom: 34, borderTopLeftRadius: 28, borderTopRightRadius: 28, backgroundColor: "#fff" },
  ...
  sheetAction: { minHeight: 54, paddingHorizontal: 10, flexDirection: "row", alignItems: "center", gap: 15, borderRadius: 15 },
