...
function safeFileName(image: WhatsAppViewerImage) {
  ...
  const fallback = `whatsapp-image.${fallbackExtension}`;
  return (image.fileName || fallback).replace(/[^a-zA-Z0-9._-]+/g, "-");
}
...
export function WhatsAppImageViewer({ image, token, onClose }: Props) {
  ...
  const resetTransform = useCallback((animated = true) => {
    const next = animated ? withSpring(1, { damping: 18, stiffness: 220 }) : 1;
    scale.value = next;
  ...
  }, [image?.assetId, image?.url, resetTransform]);
  ...
  const downloadImage = useCallback(async () => {
    ...
    const freshAsset = image.assetId && token
      ? await getWaAsset(token, image.assetId)
      : null;
    const url = freshAsset?.url || image.url;
    ...
    >
      <View style={styles.container}>
        ...
        <View style={[styles.bottomBar, { paddingBottom: insets.bottom + 14 }]}>
          <Pressable accessibilityRole="button" style={styles.actionButton} disabled={action !== null} onPress={saveImage}>
            {action === "save"
              ? <ActivityIndicator size={20} color="#ffffff" />
              : <MaterialCommunityIcons name="download-outline" size={23} color="#ffffff" />}
            <Text style={styles.actionText}>Save</Text>
          </Pressable>
          <Pressable accessibilityRole="button" style={styles.actionButton} disabled={action !== null} onPress={shareImage}>
            {action === "share"
              ? <ActivityIndicator size={20} color="#ffffff" />
