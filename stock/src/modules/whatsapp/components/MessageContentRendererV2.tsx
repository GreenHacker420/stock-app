...
function AssetUnavailable({ message }: RendererProps) {
  ...
  if (assetStatus === "FAILED" || message.operationState === "TERMINALLY_FAILED") return <InfoRow icon="cloud-alert-outline" text="Media processing failed" muted />;
  if (assetStatus === "DELETED") return <InfoRow icon="delete-outline" text="Media no longer available" muted />;
  ...
  return (
    <View>
      <Pressable
        ...
        onPress={() => onOpenImage?.({ assetId: message.asset?.id, url, fileName: message.asset?.fileName, mimeType: message.asset?.mimeType, width: message.asset?.width, height: message.asset?.height })}
      >
        <View style={[styles.imageFrame, frame]}>
          <Image source={{ uri: url }} style={styles.image} contentFit="cover" cachePolicy="memory-disk" recyclingKey={message.id} transition={0} onError={() => setFailed(true)} />
          {failed && (
...
const VideoRenderer = memo(function VideoRenderer({ message }: RendererProps) {
  ...
  return (
    <>
      {opened ? <MountedVideoPlayer url={url} /> : (
  ...
  return (
    <>
      <Pressable accessibilityRole="button" accessibilityLabel="Open document" onPress={() => openUrl(message.asset!.url, "This document is not available.")}>
        <InfoRow icon="file-document-outline" text={message.asset.fileName || message.content?.filename || "Document"} detail={message.asset.size ? formatFileSize(message.asset.size) : undefined} actionIcon="open-in-new" />
      </Pressable>
      {!!message.content?.caption && <Text selectable style={styles.messageText}>{message.content.caption}</Text>}
    </>
  );
...
function InteractiveRenderer({ message }: RendererProps) { return <InfoRow icon="gesture-tap-button" text={message.content?.body || message.content?.title || message.content?.text || "Interactive response"} detail={message.content?.description} />; }
function FlowRenderer({ message }: RendererProps) { return <InfoRow icon="form-select" text={message.content?.body || message.content?.title || "Flow response"} detail={message.payload?.subtype} />; }
...
function SystemRenderer({ message }: RendererProps) { return <InfoRow icon="information-outline" text={message.content?.body || message.content?.type || "WhatsApp system message"} muted />; }
