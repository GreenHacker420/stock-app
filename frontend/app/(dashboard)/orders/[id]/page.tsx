...
export default function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div data-keyboard-scope={scope}>
      <WorkspacePage>
        <WorkspacePageHeader kicker="Records · Order" title={order.orderNumber} description="Backend order state, line quantities, payment position and fulfilment timeline. No UI-only status is synthesized." backHref={null} onBack={goBack} icon={ShoppingBag} meta={<><Badge variant="secondary" className="text-[9px]">{order.status.replaceAll("_", " ")}</Badge><Badge variant="outline" className="text-[9px]">{order.priority}</Badge></>} actions={<Button variant="outline" size="sm" className="h-9 gap-1.5" onClick={() => void query.refetch()}><RefreshCw className="size-3.5"/>Refresh</Button>} />
