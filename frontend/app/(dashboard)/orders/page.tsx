...
const PAGE_SIZE = 50;
const REPORT_ID = "orders.register";
...
export default function OrdersPage() {
  ...
  const totals = useMemo(() => (query.data ?? []).reduce((acc, order) => {
  ...
  const columns = useMemo<ColumnDef<OrderRegisterRow>[]>(() => [
  ...
  const openOrder = useCallback((order: OrderRegisterRow) => {
    drilldownStack.push({
      route: pathname,
  ...
  const searchEscapeCommand = useMemo(() => ({
    id: "orders.register.search.close",
    title: "Return to Order Register",
  ...
  useKeybinding(useMemo(() => ({ id: "orders-register-search-escape", key: "esc", command: searchEscapeCommand.id, when: "orders.search && report.id == orders.register && input.editable && !dialog.open", priority: 170 }), [searchEscapeCommand.id]));

  const workspaceScope = JSON.stringify({ "app.module": "orders", "app.view": REPORT_ID, "orders.focused": true, "keyboard.scope": "workspace" });
  const searchScope = JSON.stringify({ "report.focused": true, "report.id": REPORT_ID, "orders.search": true, "keyboard.scope": "report.search" });

  return (
    <div data-keyboard-scope={workspaceScope}>
      <WorkspacePage>
        <WorkspacePageHeader kicker="Records · Fulfilment" title="Order register" description="Customer orders, reservation lifecycle and fulfilment state. Status values map directly to the backend OrderStatus enum." icon={ShoppingBag} actions={<><Button variant="outline" size="sm" className="h-9 gap-1.5" onClick={() => void query.refetch()}><RefreshCw className="size-3.5" />Refresh</Button><FeatureActionButton featureId="ORDER_CREATE" icon={ShoppingBag} /></>} />
        ...
        <WorkspacePanel title="Fulfilment queue" description="Text search filters only the current server page. Status and business period are sent to the backend.">
          <WorkspaceToolbar>
            <div className="relative w-[clamp(13rem,26vw,30rem)] max-w-full flex-1 sm:flex-none" data-keyboard-scope={searchScope}>
              <Search className="absolute left-3 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" />
              <Input ref={searchRef} value={pageFilter} onChange={(event) => setPageFilter(event.target.value)} placeholder="Filter this page by order # or customer…" className="h-9 bg-background pl-9 text-xs" aria-label="Filter Order Register" aria-keyshortcuts="Control+F" />
            </div>
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="w-[min(82vw,15rem)]"><DropdownMenuLabel>Order status</DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuItem onClick={() => setParams({ status: null, page: null })}>All statuses</DropdownMenuItem>{ORDER_STATUSES.map((item) => <DropdownMenuItem key={item} onClick={() => setParams({ status: item, page: null })}>{item.replaceAll("_", " ")}</DropdownMenuItem>)}</DropdownMenuContent>
            </DropdownMenu>
            <Badge variant="outline" className="h-8 text-[10px]">{startDate} → {endDate}</Badge>
          </WorkspaceToolbar>

          <OperationalDataTable id={REPORT_ID} data={rows} columns={columns} getRowId={(order) => order.id} isLoading={query.isLoading} isError={query.isError} onRetry={() => void query.refetch()} onRowOpen={openOrder} onFilterRequest={focusSearch} autoFocus emptyTitle="No orders found" emptyDescription={pageFilter ? "No order on this page matches the filter." : "No orders were returned for the selected backend filters."} renderMobileCard={(order) => <div className="rounded-xl bg-card p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="font-mono text-[10px] text-muted-foreground">{order.orderNumber}</p><p className="mt-1 truncate text-sm font-semibold">{order.customer?.name || "Customer"}</p><p className="mt-0.5 text-[10px] text-muted-foreground">{formatDate(order.createdAt)} · {order.items.length} lines</p></div><Badge className={`text-[9px] ${orderTone(order.status)}`}>{order.status.replaceAll("_", " ")}</Badge></div><div className="mt-3 flex items-end justify-between border-t pt-2"><span className="text-[10px] text-muted-foreground">Balance {formatINR(order.balanceAmount)}</span><span className="numeric-cell text-base font-semibold">{formatINR(order.totalAmount)}</span></div></div>} />
